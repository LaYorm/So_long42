...
/*   Updated: 2025/12/22 13:18:23 by yorimek          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_which_image(t_data *data, t_map_data *map_data, int i, int j)
{
	...
	if (map_data->map[j][i] == ENEMY)
		mlx_put_image_to_window(data->mlx_ptr,
			data->win_ptr, data->enemy_img[0], i * 32, j * 32);
}
...
void	ft_init_textures(t_data *data)
{
	...
	data->enemy_img[0] = mlx_xpm_file_to_image(data->mlx_ptr, ENEMY_1, &x, &y);
	data->enemy_img[1] = mlx_xpm_file_to_image(data->mlx_ptr, ENEMY_2, &x, &y);
	data->enemy_img[2] = mlx_xpm_file_to_image(data->mlx_ptr, ENEMY_3, &x, &y);
	data->enemy_img[3] = mlx_xpm_file_to_image(data->mlx_ptr, ENEMY_4, &x, &y);
	data->enemy_img[4] = mlx_xpm_file_to_image(data->mlx_ptr, ENEMY_5, &x, &y);
	if (!data->exit | !data->floor || !data->item || !data->p_down
		|| !data->p_left || !data->p_up || !data->p_right || !data->wall
		|| !data->enemy_img[0] || !data->enemy_img[1] || !data->enemy_img[2]
		|| !data->enemy_img[3] || !data->enemy_img[4])
	{
